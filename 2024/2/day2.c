// Problema explicado na linha: 1012
...
int calculate_safety(int list[][8])
{
    int safe_reports = 0;
    int level_counter = 0;
    int report_size_marker = 0;
    int count = 0;
    ...
    for(int report = 0; report < 1000; report++) {
        ...
        for(int level = 0; level < 7; level++) {
            if(list[report][level + 1] == 0) {
                report_size_marker = level;
                break;
            } else {
                report_size_marker = 7;
            }
            count = difference(list[report][level], list[report][level + 1]);
            if(count > 3 || count < -3 || count == 0)  break;
            if(count >= -3 && count < 0)
                level_counter--;
            if(count <= 3 && count > 0)
                level_counter++;
        }
        if(level_counter == report_size_marker || level_counter == -report_size_marker) {
            safe_reports++;
        }
        report_size_marker = 0;
    }
